...
#define NX_THREADS_ONLY_CALLER_CHECKING \
    if ((TX_THREAD_GET_SYSTEM_STATE()) || (_tx_thread_current_ptr == TX_NULL) || (_tx_thread_current_ptr == &_tx_timer_thread)) return (NX_CALLER_ERROR);

#define NX_INIT_AND_THREADS_CALLER_CHECKING                                                                                                        \
    if (((TX_THREAD_GET_SYSTEM_STATE()) && (TX_THREAD_GET_SYSTEM_STATE() < ((ULONG)0xF0F0F0F0))) || (_tx_thread_current_ptr == &_tx_timer_thread)) \
        return (NX_CALLER_ERROR);

#define NX_NOT_ISR_CALLER_CHECKING \
    if ((TX_THREAD_GET_SYSTEM_STATE()) && (TX_THREAD_GET_SYSTEM_STATE() < ((ULONG)0xF0F0F0F0))) return (NX_CALLER_ERROR);

#define NX_THREAD_WAIT_CALLER_CHECKING                                                                                                             \
    if ((wait_option) && ((_tx_thread_current_ptr == NX_NULL) || (TX_THREAD_GET_SYSTEM_STATE()) || (_tx_thread_current_ptr == &_tx_timer_thread))) \
        return (NX_CALLER_ERROR);
...
#define NX_THREADS_ONLY_CALLER_CHECKING \
    if ((TX_THREAD_GET_SYSTEM_STATE()) || (_tx_thread_current_ptr == TX_NULL)) return (NX_CALLER_ERROR);
...
#define NX_THREAD_WAIT_CALLER_CHECKING \
    if ((wait_option) && ((_tx_thread_current_ptr == NX_NULL) || (TX_THREAD_GET_SYSTEM_STATE()))) return (NX_CALLER_ERROR);
...
CHAR _nx_version_id[] = "Copyright (c) Microsoft Corporation. All rights reserved.  *  NetX Duo Cortex-M0/GNU Version 6.2.1 *";
#else
